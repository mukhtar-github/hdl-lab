...
- `experiments/0001` sweep, and the default build at this commit. GCC's `-foptimize-crc` is on at -O2: the CRC that runs is a 512-byte lookup table in `.rodata`, wrapped in bit-reflection code — not the loop in the source.
