# spike-crc-rv32im-O2-tune-sifive7

- **Captured (UTC):** 20260925T084337Z
- **Command:** `make -C bench/rv32 check TUNE=sifive-7-series`
...
- **Overrides:** TUNE=sifive-7-series. Everything else is the Makefile at the commit above.
...
`instret = 00097091` (618,641) · `crc_ref = 0x4cd4` · `acc = 0x33cc525d` · `iters = 0x3e8` · oracle **PASS**

## Notes

- Only `-mtune` changed. The compiler's default tuning (`rocket`) is configuration: a GCC 16.2.0 configured with this default would print the same version line.
